...
jQuery(function(){jQuery(".translate_block").show();var a=document.getElementById("translate_popup");if(a){document.body.appendChild(a)}});function google_translate(d,c,e){var a=document.getElementById((("comment"==c)?"div-":"")+c+"-"+e),b;if(!a&&"post"==c){a=document.getElementById("content_div-"+e)}if(!a&&"comment"==c){a=document.getElementById("comment-"+e)}if(a){b="#translate_loading_"+c+"-"+e;jQuery(a).translate(d,{fromOriginal:true,not:".translate_block",start:function(){jQuery(b).show()},complete:function(){jQuery(b).hide()},error:function(){jQuery(b).hide()}})}jQuery("#translate_popup").slideUp("fast")}function localize_languages(d,c){var e=jQuery("#translate_popup a").get(),a=[],b;for(b in e){a[b]=e[b].title}jQuery.translate(a,"en",d,{complete:function(){a=this.translation;for(b in e){e[b].title=a[b]}jQuery(c).data("localized",true)}})}function show_translate_popup(i,d,a){var h=document.getElementById("translate_popup"),c=jQuery(h),g=jQuery("#translate_button_"+d+"-"+a),f=Math.round(g.offset().left),e=Math.round(g.offset().top+g.outerHeight(true)),b;if(h){jQuery.translate.languageCodeMap.pt="pt-PT";if("none"==h.style.display||c.position().top!=e){h.style.display="none";c.css("left",f).css("top",e);c.slideDown("fast");b=jQuery(window).width()+jQuery(window).scrollLeft()-c.offset().left-c.outerWidth(true);if(b<0){c.css("left",Math.max(0,c.offset().left+b))}jQuery("#translate_popup .languagelink").each(function(){jQuery(this).unbind("click").click(function(){google_translate(this.lang,d,a);return false})});if("en"!=i&&(!c.data("localized"))){localize_languages(i,h)}}else{c.slideUp("fast")}}};
